...
- Create a `src/` package layout or document why the project is intentionally simpler.
- Add a README that explains the project purpose and first run command.
- Keep examples, tests, and docs in predictable top-level directories.
...
- Choose the Python version range intentionally.
- Add a project metadata file.
- Document the install and run commands.
...
- Add a package script or task command.
- Make CI fail when formatting is not applied.
...
- Decide whether type checking is required for all code or only source modules.
- Add the type-check command.
- Include type checking in CI when the project is ready.
...
- Keep the workflow readable enough for contributors to debug.
...
- Link to deeper docs only when they exist.
...
- Mention licensing in the README.
...
- Add a code of conduct when inviting public collaboration.
- Add contributing guidelines.
- Explain how issues and pull requests should be opened.
...
- Document setup, test, lint, and build commands.
