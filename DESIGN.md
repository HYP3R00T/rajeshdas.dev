...
Listing routes may use the same component with a level-one heading for their page introduction. On the Articles route, follow that introduction with a compact category-filter toolbar and one continuous three-column archive grid separated by one-pixel rules rather than detached rounded cards. Categories form a small, controlled editorial taxonomy—currently Product, Engineering, and Guides—while tags remain flexible content metadata and do not drive the primary archive navigation.
...
Article cards use a deliberately compact internal reading order: metadata, a consistent gap, title, optional description, then a bottom-aligned action. Titles and descriptions use responsive line clamps: copy may occupy available space, but it must truncate with an ellipsis before it can distort the media-to-content relationship or create uneven card rows.

Render the selected article as the first grid item and distinguish its content surface with the accent color and high-contrast text. It remains the same size as its neighbors, so featured status does not create a separate section or duplicate the archive hierarchy. Keep the featured surface stable on hover; title and reading-action feedback are sufficient.
...
Article archive cards are text-led and intentionally omit cover images. Each equal-height cell uses publication metadata, a clearly separated title and description, and a bottom-aligned reading action. This quieter composition preserves negative space and keeps the editorial hierarchy dominant. The selected article applies the accent treatment to the whole card because no separate media surface remains.
...
Individual note routes reuse the article-detail one-to-three reading architecture but omit the cover surface entirely. The wider reading track begins directly with padded prose, while the sticky rail retains archive navigation, note metadata, sharing, and the table of contents. Returning from a note preserves the reader's last topic-filtered archive route when available.
