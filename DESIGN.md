...
Article cards borrow the homepage selected-work card hierarchy: full-width cover media, title, restrained description, and a dedicated bottom row for publication metadata and the internal-navigation control. The title and description use the full content width rather than sharing a grid track with the action. Covers use a stable widescreen crop and a very small hover zoom; the card itself keeps the shared media-card border and radius. Titles use a two-line clamp and descriptions may use up to three lines, preserving useful copy before truncation while the bottom row stays aligned across cards.

Render the selected article as the first grid item and identify it only with a quiet `Featured article` text label in the bottom metadata row. Featured and standard cards use the same surface, neutral border, size, and interaction treatment; featured status must not introduce an accent background or border, create a separate section, duplicate the archive hierarchy, or overpower the cover image. Vertically center the label, date, and navigation control within that bottom row. Title, media zoom, and the internal-navigation control provide hover feedback.

Featured article selection is category-aware. The unfiltered archive promotes the newest article explicitly marked as featured; after filtering, the newest featured article within that category moves to the first cell and receives the same text label. Never infer featured status from recency alone. If a category contains no explicitly featured article, its archive begins chronologically with no featured label.
